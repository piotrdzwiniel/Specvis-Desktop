...
import org.specvis.datastructures.settings.ProcedureBasicSettingsFixMonitorBoth;
...
public class ProcedureBasicFixMonitorBoth extends Stage {
...
//    private ProcedureBasicSettingsFixMonitorFixPointChange settingsFixMonFixPointChange;
    private ProcedureBasicSettingsFixMonitorBoth settingsFixMonitorBoth;
    ...
    private ArrayList<Boolean> arrayListAnswersToFixMonitor_Blindspot;
    private ArrayList<Boolean> arrayListAnswersToFixMonitor_FixPointChange;
    ...
    private String fixationMonitorTurn; // "Blindspot" and "Fixation point change"

    /**
     * Constructor for the procedure.
     */
    public ProcedureBasicFixMonitorBoth() {
                        ...
                        if (youCanRespondToStimulus) {
                            ...
                            if (fixationMonitorIterator != fixationMonitorCheckRate) {
                                ...
                                if (fixationMonitorTurn.equals("Blindspot")) {
                                    arrayListAnswersToFixMonitor_Blindspot.add(true);
                                } else {
                                    /* If fixationMonitorTurn.equals("Fixation point change") */
                                    arrayListAnswersToFixMonitor_FixPointChange.add(true);
                                }
                            }
...
//        settingsFixMonFixPointChange = new ProcedureBasicSettingsFixMonitorFixPointChange();
        settingsFixMonitorBoth = new ProcedureBasicSettingsFixMonitorBoth();
        ...
        if (settingsFixMonitorBoth.isMonitorFixationEveryXStimuli()) {
            fixationMonitorCheckRate = settingsFixMonitorBoth.getMonitorFixationEveryXStimuli();
        } else {
            fixationMonitorCheckRate = functions.randomInt(
                    settingsFixMonitorBoth.getMonitorFixationEveryXToYStimuli_1(),
                    settingsFixMonitorBoth.getMonitorFixationEveryXToYStimuli_2()
            );
        }

        arrayListAnswersToFixMonitor_Blindspot = new ArrayList<>();
        arrayListAnswersToFixMonitor_FixPointChange = new ArrayList<>();
    ...
    private void initAndRunFixationMonitorTimeline() {
        ...
        if (fixationMonitorTurn.equals("Blindspot")) {
            initFixationMonitorShape_Blindspot();
        } else {
            /* If fixationMonitorTurn.equals("Fixation point change") */
            initFixationMonitorShape_FixPointChange();
        }
    ...
    private void initAndRunMsgAfterFixLossTimeline() {
        ...
        /* Update cumulative time interval */
        cumulativeTimeInterval += settingsFixMonitorBoth.getResumeProcedureAutomaticallyAfterXMs();
        ...
        /* Update cumulative time interval */
        cumulativeTimeInterval += settingsFixMonitorBoth.getShowNextStimAfterMsgAfterXMs();
    ...
    private void initMessageAfterFixationLoss() {
        ...
        msgBox.setText(settingsFixMonitorBoth.getTextOfTheMsg());

        double msgBoxWidthInPx = settingsFixMonitorBoth.getMsgBackgroundSizeInDgX() * pxForOneDgX;
        double msgBoxHeightInPx = settingsFixMonitorBoth.getMsgBackgroundSizeInDgY() * pxForOneDgY;

        double msgBoxLocationX = centerOfTheGridInPxX + (settingsFixMonitorBoth.getMsgDistanceFromFixPointInDgX() * pxForOneDgX) - (msgBoxWidthInPx / 2);
        double msgBoxLocationY = centerOfTheGridInPxY + (settingsFixMonitorBoth.getMsgDistanceFromFixPointInDgY() * pxForOneDgY) - (msgBoxHeightInPx / 2);
        ...
        String style = "-fx-border-width: 2; " +
                ...
                "-fx-background-color: " + functions.toHexCode(settingsFixMonitorBoth.getMsgBackgroundColor()) + "; " +
                "-fx-text-fill: " + functions.toHexCode(settingsFixMonitorBoth.getFontColor()) + "; " +
                "-fx-font-weight: " + settingsFixMonitorBoth.getFontWeight() + ";" +
                "-fx-font-size: " + settingsFixMonitorBoth.getFontSize() + "px;";
    ...
    public void decideAboutStimulusDeactivation(int[] answers) {
        ...
        if (deactivateThisStimulus) {
            ...
            if (arrayListActiveStimuliIndices.size() == 0) {
                ...
                int totalFixMonChecks_Blindspot = arrayListAnswersToFixMonitor_Blindspot.size();
                int totalFixMonChecks_FixPointChange = arrayListAnswersToFixMonitor_FixPointChange.size();

                int posFixMonChecks_Blindspot = (int) arrayListAnswersToFixMonitor_Blindspot.stream().filter(f -> !f).count();
                int posFixMonChecks_FixPointChange = (int) arrayListAnswersToFixMonitor_FixPointChange.stream().filter(f -> f).count();

                double fixMonAccuracyInPerc_Blindspot = functions.round(((double) posFixMonChecks_Blindspot / totalFixMonChecks_Blindspot) * 100, 2);
                double fixMonAccuracyInPerc_FixPointChange = functions.round(((double) posFixMonChecks_FixPointChange / totalFixMonChecks_FixPointChange) * 100, 2);

                /* Write results in text area in Procedure Preview View */
                viewProcedurePreviewController.addTextToTextArea("Total B fixation checks: " + totalFixMonChecks_Blindspot + "\n");
                viewProcedurePreviewController.addTextToTextArea("Positive B fixation checks: " + posFixMonChecks_Blindspot + "\n");
                viewProcedurePreviewController.addTextToTextArea("B fixation accuracy (%): " + fixMonAccuracyInPerc_Blindspot + "\n\n");

                viewProcedurePreviewController.addTextToTextArea("Total FPC fixation checks: " + totalFixMonChecks_FixPointChange + "\n");
                viewProcedurePreviewController.addTextToTextArea("Positive FPC fixation checks: " + posFixMonChecks_FixPointChange + "\n");
                viewProcedurePreviewController.addTextToTextArea("FPC fixation accuracy (%): " + fixMonAccuracyInPerc_FixPointChange + "\n\n");
                ...
                procedureBasicData.setTotalFixationMonitorChecks(totalFixMonChecks_Blindspot + totalFixMonChecks_FixPointChange);
                procedureBasicData.setPositiveFixationMonitorChecks(posFixMonChecks_Blindspot + posFixMonChecks_FixPointChange);
                procedureBasicData.setFixationMonitorAccuracyInPercentages((fixMonAccuracyInPerc_Blindspot + fixMonAccuracyInPerc_FixPointChange) / 2);
