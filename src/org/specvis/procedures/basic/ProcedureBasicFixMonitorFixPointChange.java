...
public class ProcedureBasicFixMonitorFixPointChange extends Stage {
     ...
     */
    public ProcedureBasicFixMonitorFixPointChange() {
    ...
    private void initLocalFields() {
        ...
        if (settingsFixMonFixPointChange.isMonitorFixationEveryXStimuli()) {
            fixationMonitorCheckRate = settingsFixMonFixPointChange.getMonitorFixationEveryXStimuli();
        } else {
            fixationMonitorCheckRate = functions.randomInt(
                    settingsFixMonFixPointChange.getMonitorFixationEveryXToYStimuli_X(),
                    settingsFixMonFixPointChange.getMonitorFixationEveryXToYStimuli_Y()
            );
        }
    ...
    private void initAndRunFixationMonitorTimeline() {
        ...
        KeyFrame keyFrameIntervalBetweenStimuli = new KeyFrame(Duration.millis(cumulativeTimeInterval), event -> {
            ...
            if (youCanRespondToStimulus) {
                ...
                if (settingsFixMonFixPointChange.isShowMessage()) {
    ...
    private void initAndRunMsgAfterFixLossTimeline() {
        ...
        /* Update cumulative time interval */
        cumulativeTimeInterval += settingsFixMonFixPointChange.getResumeProcedureAutomaticallyAfterXMs();
        ...
        /* Update cumulative time interval */
        cumulativeTimeInterval += settingsFixMonFixPointChange.getNextStimulusAfterXMsAfterMessage();
    ...
    private void initMessageAfterFixationLoss() {
        ...
        msgBox.setText(settingsFixMonFixPointChange.getMessageText());

        double msgBoxWidthInPx = settingsFixMonFixPointChange.getMessageBackgroundSizeInDegreesHorizontal() * pxForOneDgX;
        double msgBoxHeightInPx = settingsFixMonFixPointChange.getMessageBackgroundSizeInDegreesVertical() * pxForOneDgY;

        double msgBoxLocationX = centerOfTheGridInPxX + (settingsFixMonFixPointChange.getMessageDistanceFromFixPointInDegreesHorizontal() * pxForOneDgX) - (msgBoxWidthInPx / 2);
        double msgBoxLocationY = centerOfTheGridInPxY + (settingsFixMonFixPointChange.getMessageDistanceFromFixPointInDegreesVertical() * pxForOneDgY) - (msgBoxHeightInPx / 2);
        ...
        String style = "-fx-border-width: 2; " +
                ...
                "-fx-background-color: " + functions.toHexCode(settingsFixMonFixPointChange.getMessageBackgroundColor()) + "; " +
                "-fx-text-fill: " + functions.toHexCode(settingsFixMonFixPointChange.getMessageFontColor()) + "; " +
                "-fx-font-weight: " + settingsFixMonFixPointChange.getMessageFontWeight() + ";" +
                "-fx-font-size: " + settingsFixMonFixPointChange.getMessageFontSize() + "px;";
    ...
    public void decideAboutStimulusDeactivation(int[] answers) {
        ...
        if (deactivateThisStimulus) {
            ...
            if (arrayListActiveStimuliIndices.size() == 0) {
                ...
                viewProcedurePreviewController.addTextToTextArea("Total FPC fixation checks: " + totalFixMonChecks + "\n");
                viewProcedurePreviewController.addTextToTextArea("Positive FPC fixation checks: " + posFixMonChecks + "\n");
                viewProcedurePreviewController.addTextToTextArea("FPC fixation accuracy (%): " + fixMonAccuracyInPerc + "\n\n");
